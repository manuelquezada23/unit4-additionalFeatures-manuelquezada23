...
export const FrontendLinkGateway = {
  ...
  deleteLink: async (linkId: string): Promise<IServiceResponse<null>> => {
    try {
   ...
   * - delete anchors (frontend AnchorGateway)
   *
   * @param linkIds linkIds of the links to delete
   * @returns Promise<IServiceResponse<null>>
   */
  deleteLinks: async (linkIds: string[]): Promise<IServiceResponse<null>> => {
    try {
      ...
      // delete links
      const deleteLinksResp = await post<IServiceResponse<null>>(
  ...
  getLinksByAnchorId: async (
    ...
    try {
      ...
      return failureServiceResponse(
        "[getLinksByAnchorId] Unable to access backend"
      );
    }
  },
  ...
  getLinksByAnchorIds: async (
    ...
    try {
      ...
      return failureServiceResponse(
        "[getLinksByAnchorIds] Unable to access backend"
      );
    }
  },
};
