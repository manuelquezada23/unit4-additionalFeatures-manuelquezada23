...
export const loadAnchorToLinksMap = async (props: LocalINodeLinkMenuProps) => {
  ...
  const awaitLinks = async () => {
    ...
    for (let i = 0; i < anchors.length; i++) {
      linkPromises.push(
        FrontendLinkGateway.getLinksByAnchorId(anchors[i].anchorId)
      );
    }
      ...
      if (currAnchorLinks !== null) {
        ...
        for (let j = 0; j < currAnchorLinks.length; j++) {
          ...
          if (anchors[i].anchorId == currLink.anchor1Id) {
            nodeResp = await FrontendNodeGateway.getNode(
              currLink.anchor2NodeId
            );
            anchorResp = await FrontendAnchorGateway.getAnchor(
              currLink.anchor2Id
            );
          } else {
            nodeResp = await FrontendNodeGateway.getNode(
              currLink.anchor1NodeId
            );
            anchorResp = await FrontendAnchorGateway.getAnchor(
              currLink.anchor1Id
            );
