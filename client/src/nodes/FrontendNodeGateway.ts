...
export const FrontendNodeGateway = {
  ...
  deleteNode: async (nodeId: string): Promise<IServiceResponse<object>> => {
    try {
      ...
      // delete node
      return await remove<IServiceResponse<INode>>(
        baseEndpoint + servicePath + nodeId
      );
    } catch (exception) {
