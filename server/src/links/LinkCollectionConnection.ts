...
export class LinkCollectionConnection {
   ...
   */
  async clearLinkCollection(): Promise<IServiceResponse<null>> {
   ...
   *         failureServiceResponse on failure
   */
  async deleteLink(linkId: string): Promise<IServiceResponse<null>> {
   ...
   *         failureServiceResponse on failure
   */
  async deleteLinks(linkIds: string[]): Promise<IServiceResponse<null>> {
    const collection = await this.client.db().collection(this.collectionName);
   ...
   *         failureServiceResponse on failure
   */
  async findLinksByAnchorIds(
    anchorIds: string[]
  ): Promise<IServiceResponse<ILink[]>> {
    const foundLinks = [];
    const myQuery = {
      $or: [
        { anchor1Id: { $in: anchorIds } },
        { anchor2Id: { $in: anchorIds } },
      ],
    };
    await this.client
      .db()
