...
export class BackendNodeGateway {
  ...
  async deleteNode(nodeId: string): Promise<IServiceResponse<null>> {
    ...
    if (!nodeResponse.success) {
      return failureServiceResponse(
        "Failed to find the node you wanted to delete"
      );
    }
  ...
  async removeChild(
    ...
    const getParentResp = await this.nodeCollectionConnection.findNodeById(
      parentId
    );
    ...
    const updateOldParentResp = await this.updateNode(parent.nodeId, [
      makeINodeProperty(
        "filePath",
        makeINodePath(parent.filePath.path, newChildren)
      ),
    ]);
  ...
  async moveNode(
    ...
    const getNodeResp = await this.nodeCollectionConnection.findNodeById(
      nodeToMoveId
    );
    if (!getNodeResp.success) {
      return failureServiceResponse(
        "Node with nodeId " + nodeToMoveId + " not found"
      );
    }
    ...
    if (nodeToMove.filePath.path.length > 1) {
      const oldParentId =
        nodeToMove.filePath.path[nodeToMove.filePath.path.length - 2];
  ...
  async addChild(
    ...
    if (parent.filePath.path.includes(childId)) {
      return failureServiceResponse(
        "Child cannot be a descendant/ancestor of itself"
      );
    }
   ...
   */
  async getTreeByVertex(
    nodeId: string
  ): Promise<IServiceResponse<RecursiveNodeTree>> {
    const getResponse = await this.nodeCollectionConnection.findNodeById(
      nodeId
    );
  ...
  private async recursivelyDeleteSubtree(
    ...
    return deleteResp.success
      ? successfulServiceResponse(null)
      : failureServiceResponse(
