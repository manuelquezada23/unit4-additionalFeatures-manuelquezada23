...
describe("E2E Test: Anchor CRUD", () => {
  ...
  beforeAll(async () => {
    ...
    backendAnchorGateway = new BackendAnchorGateway(
      mongoClient,
      collectionName
    );
    await mongoClient.connect();

    const getResponse = await backendAnchorGateway.getAnchorById(
      testAnchor.anchorId
    );
  ...
  test("retrieves anchor", async () => {
    const response = await backendAnchorGateway.getAnchorById(
      testAnchor.anchorId
    );
    expect(response.success).toBeTruthy();
    expect(response.payload.anchorId).toEqual(testAnchor.anchorId);
    expect(
      isSameExtent(response.payload.extent, testAnchor.extent)
    ).toBeTruthy();
  });

  test("deletes anchor", async () => {
    const deleteResponse = await backendAnchorGateway.deleteAnchor(
      testAnchor.anchorId
    );
