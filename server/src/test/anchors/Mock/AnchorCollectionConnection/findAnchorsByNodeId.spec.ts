...
describe("Unit Test: findAnchorByNodeId", () => {
  ...
  let anchorCollectionConnection: AnchorCollectionConnection;
  ...
  test("gets anchors when given valid nodeId", async () => {
    ...
    const createResponse1 = await anchorCollectionConnection.insertAnchor(
      validAnchor1
    );
    expect(createResponse1.success).toBeTruthy();
    const validAnchor2 = makeIAnchor("anchorId2", "nodeId", textExtent);
    const createResponse2 = await anchorCollectionConnection.insertAnchor(
      validAnchor2
    );
    expect(createResponse2.success).toBeTruthy();
    const findAnchorsByNodeIdResp =
      await anchorCollectionConnection.findAnchorsByNodeId("nodeId");
