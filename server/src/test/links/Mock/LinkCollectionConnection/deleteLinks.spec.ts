...
describe("Unit Test: deleteLinks", () => {
  ...
  test("successfully deletes single link", async () => {
    ...
    const deleteLinkResp = await linkCollectionConnection.deleteLinks([
      "link1",
    ]);
  ...
  test(
    "gives success if we try to delete links that " + "don't exist",
    async () => {
      const validLink: ILink = makeILink(
        "link1",
        "anchor1",
      ...
      const createResponse = await linkCollectionConnection.insertLink(
        validLink
      );
      expect(createResponse.success).toBeTruthy();
      const deleteLinkResp = await linkCollectionConnection.deleteLinks([
