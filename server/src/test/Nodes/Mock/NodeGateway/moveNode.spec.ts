...
describe("Unit Test: Move Node", () => {
  ...
  test("Valid move node", async () => {
    ...
    const wrapper3 = wrapper1.children.find(
  ...
  test("Valid move node to root", async () => {
    ...
    expect(moveNodeResp.success).toBeTruthy();
    expect(
      isSameNode(makeINode("3", ["3"], ["5"]), moveNodeResp.payload)
    ).toBeTruthy();
  ...
  test(
    "Failed to move node to become a descendant of " + "it's children",
    async () => {
  ...
  test("Cannot move node to be a parent of itself", async () => {
    ...
    expect(
      isSameFilePath(
        getNodeResp.payload.filePath,
        makeINodePath(["2", "3"], ["5"])
      )
    ).toBeTruthy();
  });

  test("Failure response sent when move not needed", async () => {
