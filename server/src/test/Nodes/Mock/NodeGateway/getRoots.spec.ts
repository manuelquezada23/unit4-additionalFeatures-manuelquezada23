...
describe("Unit Test: Get Roots", () => {
  ...
  test("gets entire tree structure", async () => {
    ...
    const wrapper1 = getResponse.payload.find(
      (wrapper) => wrapper.node.nodeId === "1"
    );
    expect(wrapper1.children.length).toBe(0);
    const wrapper2 = getResponse.payload.find(
      (wrapper) => wrapper.node.nodeId === "2"
    );
    expect(wrapper2.children.length).toBe(2);
    const wrapper3 = wrapper2.children.find(
      (wrapper) => wrapper.node.nodeId === "3"
    );
    expect(wrapper3.children.length).toBe(1);
    const wrapper4 = wrapper2.children.find(
      (wrapper) => wrapper.node.nodeId === "4"
    );
    expect(wrapper4.children.length).toBe(0);
    const wrapper5 = wrapper3.children.find(
      (wrapper) => wrapper.node.nodeId === "5"
    );
