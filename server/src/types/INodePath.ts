 ...
 * @param fp any type
 */
export function isINodePath(fp: any): fp is INodePath {
  const path = (fp as INodePath).path;
  const children = (fp as INodePath).children;
  const propsDefined = path !== undefined && children !== undefined;
  if (propsDefined) {
...
export function isSameFilePath(fp1: INodePath, fp2: INodePath): boolean {
  return (
    arrayEquals(fp1.children, fp2.children) && arrayEquals(fp1.path, fp2.path)
  );
}
