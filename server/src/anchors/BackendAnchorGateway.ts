...
export class BackendAnchorGateway {
  ...
  async createAnchor(anchor: any): Promise<IServiceResponse<IAnchor>> {
    ...
    if (anchorResponse.success) {
      return failureServiceResponse(
        "Anchor with duplicate ID already exist in database."
      );
    }
    // if everything check out, insert anchor
    const insertResp = await this.anchorCollectionConnection.insertAnchor(
      anchor
    );
    return insertResp;
  }
   ...
   */
  async deleteAnchor(anchorId: string): Promise<IServiceResponse<null>> {
   ...
   * @returns Promise<IServiceResponse<null>>
   */
  async deleteAnchors(anchorIds: string[]): Promise<IServiceResponse<null>> {
