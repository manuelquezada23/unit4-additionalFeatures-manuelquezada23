...
export class AnchorRouter {
  ...
  constructor(mongoClient: MongoClient) {
     ...
     * @param res response object to send to client
     */
    AnchorExpressRouter.delete(
      "/:anchorId",
      async (req: Request, res: Response) => {
        try {
          const anchorId = req.params.anchorId;
          const response = await this.BackendAnchorGateway.deleteAnchor(
            anchorId
          );
          res.status(200).send(response);
        } catch (e) {
    ...
    AnchorExpressRouter.post("/delete", async (req: Request, res: Response) => {
      try {
        const anchorIds = req.body.anchorIds;
        const response = await this.BackendAnchorGateway.deleteAnchors(
          anchorIds
        );
        res.status(200).send(response);
      } catch (e) {
    ...
    AnchorExpressRouter.post(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          const response = await this.BackendAnchorGateway.updateExtent(
            anchorId,
            extent
          );
          res.status(200).send(response);
